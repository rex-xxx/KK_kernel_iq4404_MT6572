...
#endif
			/* Dump memory info when framework low memory: record the biggest one */
			if (tasksize > max_mem) {
				max_mem = tasksize;	
				pid_dump = p->pid;			
			}
...
__MODULE_PARM_TYPE(debug_adj, uint);
#else
...
#define MSG_SIZE_TO_AEE 70
		...
		if (lowmem_indicator && selected_oom_score_adj <= 1) {
			lowmem_print(5, KERN_DEBUG "low memory: raise aee warning\n");
			in_lowmem = 1;
			lowmem_indicator = 0;
			//DAL_LowMemoryOn();
			lowmem_print(1, KERN_DEBUG "LowMemoryOn\n");
...
#ifdef CONFIG_MT_ENG_BUILD
		if (print_extra_info)
			lowmem_print(1, "[LMK] SIGKILL\n");
#endif
		retval = send_sig(SIGKILL, selected, 0);
#ifdef CONFIG_MT_ENG_BUILD
		if (print_extra_info)
			lowmem_print(1, "[LMK] retval: %d\n", retval);
#endif
		set_tsk_thread_flag(selected, TIF_MEMDIE);
		rem -= selected_tasksize;
	}
